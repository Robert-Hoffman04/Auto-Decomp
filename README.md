...
- The initial run now scans `build/<GAMEID>/asm`, generates `build/<GAMEID>/static_math_vars.txt`, appends detected static math variables to `config/<GAMEID>/symbols.txt`, and then re-runs configure/build to apply the symbol updates.
